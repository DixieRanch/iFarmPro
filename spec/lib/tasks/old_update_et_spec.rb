...
describe Tasks::UpdateEt do
  ...
  describe '#parse' do
    context 'doy' do
      it 'extracts doy 365', :slow do
        expect(array[6][:doy]).to eq 365
      end

      it 'extracts doy 1', :slow do
        expect(array[7][:doy]).to eq 1
      end

      it 'extracts doy 6', :slow do
        expect(array[12][:doy]).to eq 6
      end
    end

    context 'eth' do
      it 'extracts eth for doy 361', :slow do
        expect(array[2][:eth].to_f).to eq 0.03
      end

      it 'extracts eth for doy 362', :slow do
        expect(array[3][:eth].to_f).to eq 0.04
      end

      it 'extracts eth for doy 5', :slow do
        expect(array[11][:eth].to_f).to eq 0.04
      end

      it 'extracts eth for doy 6', :slow do
